...
import { Search, Moon, Sun, Globe, Menu } from 'lucide-react'
...
export default function Header({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  return (
    <motion.header
      ...
      className="h-16 bg-white/80 backdrop-blur-xl border-b border-surface-border/60 flex items-center justify-between px-4 md:px-6 sticky top-0 z-40"
    >
      <div className="flex items-center gap-3">
        {onMenuToggle && (
          <button
            onClick={onMenuToggle}
            className="md:hidden p-2 rounded-xl hover:bg-surface-muted transition-colors text-gray-600"
          >
            <Menu size={20} />
          </button>
        )}
        <div className="relative group hidden sm:block">
          <Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-primary-500 transition-colors rtl:right-3 ltr:left-3" />
          <input
            type="text"
            placeholder={t.header.search}
            className="pr-10 pl-10 py-2 bg-surface-muted/80 border border-transparent rounded-xl text-sm w-48 lg:w-72 focus:outline-none focus:ring-2 focus:ring-primary-500/15 focus:border-primary-500/30 focus:bg-white transition-all placeholder:text-gray-400 rtl:pr-10 ltr:pl-10"
          />
          <div className="absolute left-3 top-1/2 -translate-y-1/2 items-center gap-0.5 opacity-40 hidden lg:flex rtl:left-3 ltr:right-3">
