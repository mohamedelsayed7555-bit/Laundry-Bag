...
export default function Header({ onMobileMenuToggle }: { onMobileMenuToggle?: () => void } = {}) {
  ...
  return (
    <motion.header
      ...
      className="h-16 bg-white/80 backdrop-blur-xl border-b border-surface-border/60 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-40"
    >
      <div className="flex items-center gap-2 sm:gap-4">
        <button
          onClick={onMobileMenuToggle}
          className="lg:hidden p-2 rounded-xl hover:bg-surface-muted transition-colors text-gray-500"
        >
          <Menu size={20} />
        </button>
        <div className="relative group hidden sm:block">
          ...
          <input
            ...
            className="pr-10 pl-10 py-2 bg-surface-muted/80 border border-transparent rounded-xl text-sm w-48 lg:w-72 focus:outline-none focus:ring-2 focus:ring-primary-500/15 focus:border-primary-500/30 focus:bg-white transition-all placeholder:text-gray-400"
          />
          <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-40 hidden lg:flex">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="w-px h-8 bg-surface-border mx-1 hidden sm:block" />
